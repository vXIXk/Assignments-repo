...
news.forEach(el => createArticle(el));

function createArticle({ title, headerBgSrc, category, body, date }) {
      const newsItem = spawnEl("li", "news-item");
      newsItem.onclick = function () {
            this.classList.toggle("selected");
      };

      const card = spawnEl("article", "news-card");
      const newsHeader = spawnEl("header", "news-header");
      const newsMain = spawnEl("main", "news-main");
      const newsFooter = spawnEl("footer", "news-footer");

      const bgImg = spawnEl("div", "bg-img");
      bgImg.style.backgroundImage = `url(${headerBgSrc})`;

      const newsTitle = spawnEl("h2", "title", title);
      const newsCategory = spawnEl("h3", "category", category);
      const newsContent = spawnEl("p", "content", body);
      const newsDate = spawnEl("p", "date", date);

      const delBtn = spawnEl("button", "del-btn");
      delBtn.innerHTML = `<i class="fa-solid fa-ban"></i>`;
      delBtn.onclick = function () {
            this.parentElement.remove();
      };

      const likeBtn = spawnEl("button", "like-btn");
      ...
      newsHeader.append(bgImg, newsTitle);
      newsMain.append(newsCategory, newsContent);
      newsFooter.append(newsDate);
      card.append(newsHeader, newsMain, newsFooter);
      newsItem.append(card, delBtn, likeBtn);

      rootEl.append(newsItem);
}

function spawnEl(type, className, content) {
